...
class CallbackManager extends PaymentManager implements CallbackManagerInterface
{
    ...
    protected function validateIpAddress($callerIp = null)
    {
        ...
        if ($callerIp != $allowedIp && !$this->test) {
            $this->showDebug([ 'message' => 'Novalnet callback received. Unauthorised access from the IP [' . $callerIp . ']']);
    ...
    protected function checkParams()
    {
        ...
        if ($transactionLog) {
            ...
            if (!preg_match('/^' . $transactionId . '/i', $orderTid)) {
                $this->showDebug([ 'message' => 'Novalnet callback received. Order no is not valid']);

                return false;
            }
        }
        if ($this->callbackResponse['result']['status'] != 'SUCCESS') {
            $this->showDebug([ 'message' => 'Novalnet callback received. Status is not valid. Refer Order :' . $this->orderNo]);
    ...
    protected function transactionConfirmation()
    {
        ...
        $statusMessage = ['message' => "The transaction has been confirmed on on $this->currentTime"];
    ...
    protected function transactionCancellation()
    {
        ...
        $statusMessage = ['message' => "The transaction has been cancelled on $this->currentTime"];
        $this->setCallbackComments($statusMessage);
    ...
    protected function transactionRefund()
    {
        ...
        $amount = ($this->callbackResponse['transaction']['amount']) / 100 . ' ' . $this->currency;
        $transactionId = $this->parent_tid;
        $tid = $this->event_tid;

        $statusMessage = ($this->event_type == 'TRANSACTION_REFUND')
            ? ['message' => "Novalnet callback received. Refund/Bookback executed successfully for the TID: $transactionId amount: $amount on $this->currentTime. The subsequent TID: $tid"]
            : ['message' => "Novalnet callback received. Chargeback executed successfully for the TID: $transactionId amount: $amount on $this->currentTime. The subsequent TID: $tid"];
    ...
    protected function transactionUpdate()
    {
        ...
        if ($paymentStatus == 'PENDING' && $this->callbackResponse['transaction']['status'] == 'ON_HOLD') {
            ...
            $statusMessage = ['message' => "Novalnet callback received. The transaction status has been changed from pending to on hold for the TID: $transactionId on $this->currentTime."];
            ...
            if (!empty($this->event_data['transaction']['due_date'])) {
                $due_date = $this->event_data['transaction']['due_date'];
                /* translators: %1$s: tid, %2$s: amount, %3$s: due date */
                $statusMessage = ['message' => "Transaction updated successfully for the TID: $transactionId with amount $amount and due date $due_date"];
            } else {
        ...
        if (in_array($this->callbackResponse['transaction']['payment_type'], ['MULTIBANCO_CREDIT', 'INVOICE_CREDIT', 'ONLINE_TRANSFER_CREDIT', 'CASHPAYMENT_CREDIT'])) {
            ...
            if ($orderPaidAmount < $grandTotal) {
                if ($this->callbackResponse['transaction']['payment_type'] == 'ONLINE_TRANSFER_CREDIT') {
                    $statusMessage = ($totalAmount >= $grandTotal) ? $statusMessage . '<br />' . "The amount of $amount for the order $this->orderNo has been paid. Please verify received amount and TID details, and update the order status accordingly." . '<br />' : $statusMessage;
                ...
                $this->showDebug(
                    [
                    'message' => 'Novalnet callback received. Callback Script executed already. Refer Order :' . $this->orderNo,
                    ]
                );
            }
        } else {
            $statusMessage = ['message' => "Novalnet Callback Script executed successfully for the TID: $transactionId with amount $amount on $this->currentTime. Please refer PAID transaction in our Novalnet Merchant Administration with the TID: $referenceTid"];
